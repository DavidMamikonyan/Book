import bookSrc from '../../assets/images/singlebook.png';
import fundPaper from '../../assets/images/fund-paper.svg';
import fundBooks from '../../assets/images/fund-books.svg';
import fundDonate from '../../assets/images/fund-donate.svg';
import fundEarphones from '../../assets/images/fund-earphone.svg';
import userImg from './../../assets/images/userImg.png';
import shortAudio from '../../assets/30sec.mp3'
import {prodStatus} from "./data1";


let count = 300
let count2 = 999
let count3 = 600


export const data = [
    {
        id: count++,
        src: bookSrc,
        name: 'أهلا',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'أهلا',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit, Lorem ipsum dolor sit amet, consectetur adipiscing elit Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 550,
            'Donated': 550,
            'Time ends in': 12000
        },
        percents: 20
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 650,
            'Donated': 770,
            'Time ends in': 800
        },
        percents: 49
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 1200
        },
        percents: 80
    },
    {
        id: count++,
        src: bookSrc,
        name: 'The book name end',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit',
        goalDonateTime: {
            'The goal': 80,
            'Donated': 660,
            'Time ends in': 800
        },
        percents: 80
    }
]

export const fundData = [
    {
        id: count2++,
        src: fundPaper,
        jsonKey: 'signUp'
    },
    {
        id: count2++,
        src: fundBooks,
        jsonKey: 'startProject'
    },
    {
        id: count2++,
        src: fundDonate,
        jsonKey: 'pledge'
    },
    {
        id: count2++,
        src: fundEarphones,
        jsonKey: 'enjoy'
    }
];

export const audioBooks = [
    {
        id: count++,
        src: bookSrc,
        playAudio: shortAudio,
        authorName: 'Author name',
        name: 'The book name Tereza kasyan',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        playAudio: shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        playAudio: shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        playAudio: shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Statu666s',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Stat66us',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Stat666us',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Statu444s',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Sta5656tus',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Sta556tus',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'St+++atus',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Paruyr Sevak',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Statu888s',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: '6666',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status5656',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Paruyr Sevak',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status44',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Author name',
        name: 'The book name',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status3535',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    },
    {
        id: count++,
        src: bookSrc,
        // playAudio:shortAudio,
        authorName: 'Paruyr Sevak',
        name: 'The book name bxdo',
        content: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status99',
        narrator: 'Lorem ipsum',
        genre: 'Historical'
    }
]

export const projectsBooksData = [
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 250,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 100,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 400,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 500,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Status',
        goal: 500,
        donated: 0,
        deadline: '120:45:12',
        genre: 'Historical, Roman',
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },

]

export const productionBooksData = [
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        status: 'Complete',
        deadline: '12:30:15',
    },

]

export const libraryBooksData = [
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 60,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 10,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 51,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },
    {
        id: count3++,
        imageext: bookSrc,
        author: 'Author name',
        name: 'The book name Tereza kasyan',
        description: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry`s standard dummy text ever.',
        narrator: 'Lorem ipsum',
        genre: 'Historical',
        publicYear: '19 feb 2022',
        playAudio: shortAudio,
        pledgedData: [
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
            {
                img: userImg,
                name: 'User Name',
                pledgedSize: 50,
                dateOfPledged: '01 feb 2022',
            },
        ],
    },

];


export const productionFilterData = [
    {
        title: "Status",
        placeholder: 'All',
        dd_menu_items: prodStatus,
        id:"status"


    }
];
export const libraryFilterData = [
    {
        title: "Author",
        placeholder: 'All',
        dd_menu_items: prodStatus,
        id:"author"
    },
    {
        title: "Narrator",
        placeholder: 'All',
        dd_menu_items: prodStatus,
        id:"narrator"

    },
    {
        title: "Genre",
        placeholder: 'Detective and Mystery',
        dd_menu_items: prodStatus,
        id:"genre"

    },
    {
        title: "Year of publication",
        placeholder: '2022',
        dd_menu_items: prodStatus,
        id:"publicYear"

    },

];